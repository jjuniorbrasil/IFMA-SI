...
int lista_palindromo(Lista *li) {
    if (li == NULL)
        return 0;

    for (int i = 0; i < li->qtd/2; i++) {
        if (strcmp(li->dados[i].nome, li->dados[li->qtd-1-i].nome) != 0)
            return 0;
    }
...
void copiar_lista(Lista* li, Lista* li2) {
    if (li != NULL && li2 != NULL) {
        for (int i = 0; i < li->qtd; i++) {
            li2->dados[i] = li->dados[i];
        }
        li2->qtd = li->qtd;
    }
}
